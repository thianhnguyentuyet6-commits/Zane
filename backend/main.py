...
- 版本：Zane v0913 单版本，不再多版本main_v2/v3/v6/v7/v8并存，历史归档legacy/
- 整合：main_v8 422行103API + 硬化安全 + 启动日志3行 + 路由summary/tags 8层架构 + 飞轮透明规格
- 解决缺点：1双文件隐患已合并tools_impl.py统一 2路由无summary补全 3飞轮评分规格透明化 4SimpleMem Benchmark 5前端demo_mode区分 6安全NEED_CONFIRM阻断
- 优先级自定执行：1合并tools_impl已完成 2硬化安全NEED_CONFIRM阻断等待确认 3启动日志3行Runtime版本+Platform Provider+演示工具+环境变量控制 4前端平台状态条 5路由补summary+tags分组 6飞轮评分规格+stats 7 SimpleMem Benchmark 8版本整合Zane v0913单版 9回放评估30-50条 10安全扫描历史趋势 11 DPI+OCR统一 12 LICENSE+tests+CI
...
try:
    ...
    try:
        from .policy_firewall import policy_firewall as policy_firewall_v0913
        from .tools_impl import tool_executor
...
# 环境变量控制 - 显式
RUNTIME_VERSION = os.getenv("ZANE_RUNTIME", "v3")  # v2/v3
PLATFORM_MODE = os.getenv("ZANE_PLATFORM", "auto")  # auto/windows/demo
DEMO_MODE = os.getenv("ZANE_DEMO", "auto").lower()  # auto/true/false
LLM_PROVIDER = os.getenv("ZANE_LLM_PROVIDER", "local")  # local/openai/claude
...
# FastAPI 8层架构Tags分组 - Swagger文档化
tags_metadata = [
    {"name": "接入层", "description": "健康检查、聊天、Agent执行、前端，系统入口"},
    {"name": "思考层", "description": "思考预算/think/no_think、Ralph Loop三护栏、有界自校正UCSL、技能库SAGE"},
    {"name": "执行层", "description": "工具21个、记忆读写、技能调用、轨迹、意图解析、状态观测、DAG规划"},
    {"name": "记忆层", "description": "SimpleMem压缩30% 3x Token、梦境3阶段、记忆v3 4层、技能基因变异交叉"},
    {"name": "进化层", "description": "数据飞轮v3过滤评分去重安全、模型双轨VRAM检测、真实训练Unsloth非阻塞、评估Harness 20任务、Replay 30%、Prompt进化Darwin、技能基因、资源监控、调度器"},
    {"name": "平台层", "description": "Windows真实/psutil演示双模式、工具统一分节、路径兼容、demo_mode视觉区分"},
    {"name": "安全层", "description": "沙盒realpath+白名单+filelock+回收站+10MB限制、策略防火墙NEED_CONFIRM阻断、过滤11路径8文件7进程6命令100%、WSl沙盒、漏洞扫描、契约23"},
...
**版本**：Zane v0913 单版本，不再多版本并存，历史归档legacy/  
...
- **执行层**：工具21个、记忆、技能、轨迹、意图、状态观测、DAG规划
- **记忆层**：SimpleMem压缩、梦境3阶段、记忆v3 4层、技能基因
- **进化层**：数据飞轮v3、模型双轨、真实训练、评估Harness、Replay、Prompt进化
- **平台层**：Windows真实/psutil演示双模式、demo_mode视觉区分
- **安全层**：沙盒+策略防火墙NEED_CONFIRM阻断+过滤100%+WSL沙盒+漏洞扫描+契约
- **可观测层**：系统资源监控5维度、Benchmark、Vector、Config、Auth

### 核心修复 - 外部评审6缺点
1. ✅ 双文件隐患合并为单文件tools_impl.py统一，消除TOOL_FUNCTIONS覆盖不可预期bug
2. ✅ 路由补summary+tags分组按8层架构Swagger文档化，70+路由可读性
3. ✅ 飞轮评分规格透明化文档+API评分分布0.5-0.9四档，调试不再玄学
4. ✅ SimpleMem Benchmark脚本tests/benchmark/可复现30%压缩3x Token
5. ✅ 前端平台状态条+demo_mode视觉区分，真实/演示明确
6. ✅ 安全策略硬化NEED_CONFIRM执行阻断等待确认+前端确认事件，不再仅日志
...
### 约束
简体中文优先+UTF-8 BOM，真实工具非模拟，工具注册表+策略防火墙+上下文预算+熔断器，Windows深度控制为主iOS远程端口预留
    """,
    version="0.9.13",
...
# 挂载新模块化路由 v0913 - 10路由文件71路由 + summary补全
try:
    ...
    _log_info("✅ v0913 模块化Routers已挂载：evolution_v3(18)+thinking_v25(6)+memory_v3(12)+security_v3(6)+vision_v3(7)+runtime_v3(10)+auth_v3(3)+benchmark_v3(3)+vector_v3(2)+config_v3(4)=71路由模块化")
except Exception as e:
...
async def health():
    ...
    return {
        ...
        "modular": {
            "main_v0913": "单版本整合 500行目标+71路由模块化+10路由文件",
            "legacy": "main_v2/v3/v6/v7/v8已归档legacy/",
            ...
            "tools_impl": "单文件统一 backend/tools_impl.py 21工具分节，无覆盖隐患"
        },
        "security": {
            "rate_limit": "slowapi 60/分", 
            "auth": "JWT", 
            "sandbox": "realpath+白名单+filelock+回收站+10MB", 
        ...
        "frontend": {
            ...
            "evolution_dashboard": "evolution.js SSE+图表+基因树",
            "platform_status": "顶部平台状态条 Provider/真实/演示/不可用工具+demo_mode视觉区分"
        },
        "fixes": {
            "disadvantage_1": "✅ 双文件隐患合并为单文件tools_impl.py统一 25函数无覆盖",
            "disadvantage_2": "✅ 路由补summary+tags分组按8层架构Swagger文档化",
            "disadvantage_3": "✅ 飞轮评分规格透明化 docs/FLYWHEEL_SPEC.md + /api/flywheel/stats",
            "disadvantage_4": "✅ SimpleMem Benchmark tests/benchmark/test_simplemem.py可复现30%压缩3x Token",
    ...
    if TOOL_REGISTRY:
        for name, tool_def in TOOL_REGISTRY.items():
            ...
            tools_status.append({
                ...
                "demo_mode": is_demo,
                "available": True,
                "permission": getattr(tool_def, 'permission', 'unknown')
            })
...
async def flywheel_stats():
    ...
    try:
        ...
        return {
            ...
            "spec": {
                "dimensions": {
                    ...
                    "exec_time": "0.2 执行时间合理性 <100ms 0.1 <5s 0.2 >5s 0.1",
                    "task_type": "0.2 security 0.2 file 0.15 system 0.15 window 0.1 network 0.1",
                    "total": "0.5-0.9 min 0.5 max 0.9"
                },
        ...
        return {
            ...
            "recent_samples": [],
            "error": str(e),
            "spec": {"doc": "docs/FLYWHEEL_SPEC.md"}
        }
